---
name: "Brencis"
role: "Lord of Vampires"
description: "Brencis is the antagonist of The Blood of Dawnwalker — an ancient Roman-born vampire who rules Vale Sangora and levies a blood tax on the living."
faction: "vampire"
status: "confirmed"
quote: "Born in the Roman Empire; a thousand years later, the master of the valley."
image: "/images/brencis-and-vampire-officers.jpg"
imageAlt: "A gothic castle silhouette under a blood-red sky"
facts:
  - label: "Role"
    value: "Primary antagonist"
  - label: "True name"
    value: "Caeso Burrienus Laurentius"
  - label: "Born"
    value: "Roman Empire, AD 131"
  - label: "Rules"
    value: "Vale Sangora (the blood tax)"
  - label: "Commands"
    value: "A circle of vampire officers"
tags: ["brencis", "vampire", "antagonist"]
order: 2
publishDate: 2026-06-04
updatedDate: 2026-06-04
sources:
  - label: "Story, Setting & Characters guide"
    url: "https://dawnwalker.wiki/guides/story-setting-characters/"
  - label: "Wikipedia — The Blood of Dawnwalker"
    url: "https://en.wikipedia.org/wiki/The_Blood_of_Dawnwalker"
---

**Brencis** is the ancient vampire at the centre of the valley's misery — and, indirectly, the reason **[Coen](/characters/coen/)** becomes a Dawnwalker at all.

## A thousand-year-old predator

Brencis was born in the **Roman Empire in AD 131** as **Caeso Burrienus Laurentius**, into a wealthy noble family. Over more than a thousand years he grew into the formidable **Lord of Vampires** who now rules **Vale Sangora**. By 1347, the valley lives under his grip: he imposes a **blood tax** on the human population, and the people beneath him are sliding into servitude and despair.

## The bargain that starts the story

Brencis is not a distant final boss — he is woven into Coen's story from the first hour. When Coen's sister **Lunka** is dying of the plague, it's Brencis who **saves her with his blood** — then immediately weaponises that mercy, effectively holding Coen's family hostage and setting the roughly **30-day** clock in motion.

When he tries to turn Coen into a vampire, the **silver poisoning** in Coen's blood thwarts the transformation, creating the Dawnwalker. In a sense, Coen's curse is Brencis's mistake.

## Not one villain, but many

Crucially, Brencis doesn't rule alone. He commands a **circle of vampire officers**, which gives the game a roster of distinct, dangerous foes rather than a single climactic duel. We break down those lieutenants in our [Brencis and the vampire officers](/guides/brencis-and-vampire-officers/) guide.
